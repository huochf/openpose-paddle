...
from openpose.postprocess.body_postprocessor import BodyPostprocessor
from openpose.util import padRightDownCorner


class Body(object):

    def __init__(self, pose_points=25):
        self.pose_points = pose_points
        self.paf_dim = 28 if pose_points == 15 else pose_points * 2 + 2
        self.body_model = build_body_model(pose_points)
        self.body_model.eval()
        self.postprocessor = BodyPostprocessor(pose_points)
    ...
    def __call__(self, oriImg):
        ...
        avg_output = np.zeros((self.paf_dim + self.pose_points + 1, h, w))
